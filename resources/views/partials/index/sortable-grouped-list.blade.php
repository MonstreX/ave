@php
    ...
    // Group records if not already grouped
    if (!isset($groupedRecords) || !$groupedRecords) {
    ...
    $columnsCount = count($table->getColumns());
    $totalColumns = $columnsCount + 2; // +1 for handle, +1 for actions
    if ($hasBulkSelection) {
        $totalColumns++; // +1 for checkbox
    }
@endphp
    ...
    <div class="panel-body">
            ...
            <div class="resource-controls-left">
                ...
                @else
                    <div class="resource-note">
                        Showing all {{ $recordsTotal }} results (grouped sortable mode)
                    </div>
                @endif
        ...
        <div class="resource-table">
            <table class="table table-striped table-hover table-bordered">
                ...
                @forelse($groupedRecords as $groupId => $groupItems)
                    @php
                    ...
                    {{-- Group Header Row --}}
                    <tbody class="group-header-tbody">
                        <tr class="group-header-row">
                            <td colspan="{{ $totalColumns }}" class="group-header-cell">
                                <div class="group-header-content">
                                    <span class="group-header-title">{{ $groupName }}</span>
                                    <span class="group-header-count">({{ $groupItems->count() }})</span>
                                </div>
                            </td>
                        </tr>
                    </tbody>

                    {{-- Group Items (sortable within group) --}}
                    <tbody
                        class="group-items-tbody"
                        data-sortable="true"
                        data-group-id="{{ $groupId }}"
                        data-slug="{{ $slug }}"
                        data-group-column="{{ $groupByColumn }}"
                        data-order-column="{{ $orderColumn }}"
                        data-update-endpoint="{{ route('ave.resource.reorder', ['slug' => $slug]) }}"
                        data-group-update-endpoint="{{ route('ave.resource.updateGroup', ['slug' => $slug]) }}"
                    >
        ...
        <div class="resource-pagination">
            <div class="resource-note">
                Showing all {{ $recordsTotal }} results in {{ $groupedRecords->count() }} groups
            </div>
        </div>
    </div>
</div>

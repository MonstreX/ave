...
class DatabaseTableEditor {
    ...
    renderColumn(column, index) {
        ...
        checkboxes.style.gridTemplateColumns = 'repeat(auto-fit, minmax(150px, 1fr))'
        checkboxes.style.gap = '10px'

        // Not Null
        checkboxes.appendChild(this.createCheckbox('notnull', this.translations.notNull, column.notnull, index))

        // Unsigned (only for numeric types)
        if (type && type.category === 'numbers') {
            checkboxes.appendChild(this.createCheckbox('unsigned', this.translations.unsigned, column.unsigned, index))
        }

        // Auto Increment (only for integer types)
        if (type && (type.name === 'integer' || type.name === 'bigint' || type.name === 'smallint')) {
            checkboxes.appendChild(this.createCheckbox('autoincrement', this.translations.autoIncrement, column.autoincrement, index))
        }

        body.appendChild(checkboxes)

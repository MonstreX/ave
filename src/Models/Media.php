...
class Media extends Model
{
    ...
    public function path(): string
    {
        ...
        try {
            // Take master media file and create image instance
            $processor = new ImageProcessor();
            $processor->read($this->fileService->path($this->path, $this->disk));

            // Apply transformations
            if ($this->image['width'] && $this->image['height']) {
                // Both dimensions specified - crop/cover
                $processor->cover($this->image['width'], $this->image['height']);
            } elseif ($this->image['width'] || $this->image['height']) {
                // One dimension specified - scale with aspect ratio
                $processor->scale(
            ...
            $format = $this->image['format'] ?: $conversionData['extension'];
            $quality = $this->image['quality'];
            $encoded = $processor->encode($format, $quality);

            // Save conversion
            $this->fileService->disk($this->disk)->put(
                $conversionData['path'],
                (string) $encoded,
                $this->disk
            );

            // Save new conversion to conversions list in media entry
